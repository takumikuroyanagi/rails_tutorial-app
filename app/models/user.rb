class User < ApplicationRecord
  ...
  def authenticated?(attribute, token)
    digest = self.send("#{attribute}_digest") #self.ってしないと読み取れない
  ...
  #followed_by?メソッドを定義してもよかったのですが、サンプルアプリケーションで実際に使う場面がなかったのでこのメソッドは省略しました。

  private

    # メールアドレスをすべて小文字にする
    def downcase_email
      self.email = self.email.downcase #self.を追加
    end
